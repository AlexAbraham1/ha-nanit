"""The stop-breathing flow: PUT_STING_STOP (mirrors the app's Stop)."""
...
def _wire_transport(cam: NanitCamera) -> list[bytes]:
    ...
    resp = Response(request_id=1, request_type=RequestType.PUT_STING_STOP, status_code=200)
...
@pytest.mark.asyncio
async def test_async_stop_breathing_sends_sting_stop() -> None:
    cam = _make_camera()
    sent = _wire_transport(cam)

    await cam.async_stop_breathing_tracking()

    assert len(sent) == 1
    msg = decode_message(sent[0])
    assert msg.request.type == RequestType.PUT_STING_STOP
    assert msg.request.HasField("sting_stop")
